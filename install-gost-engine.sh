...
function is_correct_sha256() {
    ...
    while IFS=' ' read -r CRC FILE_NAME
      ...
      echo "Error comparing file name of calculating crc file with file name containing in crc file."
      return 1
}
...
OPENSSL_TAR_GZ_FILE_NAME="${OPENSSL_NAME}.${TAR}.${GZ}"
...
wget --no-check-certificate "${OPENSSL_BASE_URL}/${OPENSSL_TAR_GZ_FILE_NAME}"

OPENSSL_SHA_256_FILE_NAME="${OPENSSL_NAME}.${TAR}.${GZ}.sha256"
wget --no-check-certificate "${OPENSSL_BASE_URL}/${OPENSSL_SHA_256_FILE_NAME}"

# Не работает потом разберусь
###################################################################################
# is_correct_sha256 "./${OPENSSL_TAR_GZ_FILE_NAME}" "./${OPENSSL_SHA_256_FILE_NAME}"
# if [[ $? -eq 1 ]]
#   then
#    echo "The file '${OPENSSL_TAR_GZ_FILE_NAME}' don't match to CRC."
#    exit 1
# fi
###################################################################################
tar -xf "${OPENSSL_TAR_GZ_FILE_NAME}" -C "${SOURCE_FOLDER_PROJECT}"

mv -f "./${OPENSSL_TAR_GZ_FILE_NAME}" "${ARCHIVE_FOLDER_PROJECT}"
mv -f "./${OPENSSL_SHA_256_FILE_NAME}" "${ARCHIVE_FOLDER_PROJECT}"
